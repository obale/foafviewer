...
/**
 * Shows the Profile of an Agent. Please assure that you assign a serializable
 * object of AgentSerializable with the attribute "agent".
 * 
...
public class FOAFProfile extends Activity {
	...
	private OnItemClickListener listClickListener = new OnItemClickListener() {
		...
		public void onItemClick(AdapterView<?> list, View view, int position,
			...
			if (entry.get(TOP).equals(MAIL)) {
				...
				try {
					Drawable drawable = ImageHelper.getDrawable(agent
							.getImageURL());
					portrait.setBackgroundDrawable(drawable);
					picDialog.addContentView(portrait,
							new LinearLayout.LayoutParams(drawable
				...
				/*
				 * ContentValues event = new ContentValues(); event.put("title",
				 * "Birthday of " + agent.getName()); event.put("dtstart",
				 * DateHelper.convertStringToLong(agent.getDateOfBirth()));
				 * event.put("allDay", 1); event.put("eventStatus", 1); Uri
				...
				final Intent phoneIntent = new Intent(
				...
				final Intent profileIntent = new Intent(FOAFProfile.this,
						FOAFFriendListing.class);
				...
				final Intent websiteIntent = new Intent(
						android.content.Intent.ACTION_VIEW, Uri.parse(entry
								.get(BOTTOM)));
				startActivity(websiteIntent);
			}
		}
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		final ProgressDialog progressDialog = ProgressDialog.show(
				FOAFProfile.this, null, "Preparing FOAF Profile...", false);
		...
		Thread worker = new Thread() {
			...
			public void run() {
				...
				try {
					...
					Address address = geocoder.getFromLocation(geoPoint.first,
							geoPoint.second, 1).get(0);
					map = new HashMap<String, String>();
					map.put(ICON, R.drawable.location_icon + "");
					map.put(TOP, LOCATION);
					map.put(BOTTOM, address.getAddressLine(0) + ", "
				...
				if (scubaCertificate != null) {
					scubaCertificate = ScubaDiveHelper
							.getScubaDiveCertificate(scubaCertificate);
					map = new HashMap<String, String>();
	...
	/**
	 * Function that handles the adding of an FOAF agent to the intern contact
	 * list. TODO: Improve this code. Find workaround for fields that are not
	 * existent.
	 */
	...
	public boolean onCreateOptionsMenu(Menu menu) {
		menu.add(0, MENU_ADD_CONTACT, 20, "Add to Contacts").setIcon(
				R.drawable.addcontact_icon);
		return true;
	}
